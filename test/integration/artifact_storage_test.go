...
func TestArtifactStorageSemantics(t *testing.T) {
	...
	t.Run("LargeConditionalPutIsExclusive", func(t *testing.T) {
	...
	t.Run("PutStreamRejectsAShortBody", func(t *testing.T) {
		...
		_, err := store.PutStream(ctx, key, io.LimitReader(bytes.NewReader(randomBytes(t, 4096)), 1000), 4096, storage.PutOptions{IfNotExists: true})
		if err == nil {
	...
	t.Run("MetadataRoundTripsLowercased", func(t *testing.T) {
		...
		if _, err := store.Put(ctx, key, body, storage.PutOptions{IfNotExists: true, ContentType: capture.ContentTypePcapng, Metadata: meta}); err != nil {
			t.Fatalf("Put: %v", err)
		}
		head, err := store.Head(ctx, key)
		if err != nil {
	...
	t.Run("ManifestReadsBack", func(t *testing.T) {
		m := &capture.Manifest{
		...
		if _, err := store.Put(ctx, key, raw, storage.PutOptions{IfNotExists: true, ContentType: capture.ContentTypeManifest}); err != nil {
	...
	t.Run("DeleteIsIdempotentAndHeadSeesIt", func(t *testing.T) {
	...
	t.Run("TimeoutIsHonoured", func(t *testing.T) {
		...
		go func() {
			_, err := store.PutStream(context.Background(), key, bytes.NewReader(body), int64(len(body)), storage.PutOptions{Timeout: time.Nanosecond})
			done <- err
		}()
		select {
		case err := <-done:
