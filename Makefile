...
# state that may be hours gone.
	go test -tags=investigation -count=1 ./test/e2e/ -v -timeout 15m $(ARGS)
...
# long as it runs.
	go test -tags=acceptance -count=1 ./test/e2e/ -v -timeout 45m $(ARGS)
...
.PHONY: verify
verify: verify-tools fmt vet ## Verify tool pins and fail on generated-artifact drift.
...
# configuration at a time, so a single pass would leave the other suite unread.
	"$(GOLANGCI_LINT)" run --build-tags=investigation ./test/e2e/... ./test/integration/...
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name trawl-builder
	$(CONTAINER_TOOL) buildx use trawl-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm trawl-builder
	rm Dockerfile.cross
...
## Tool versions are pinned in hack/tools.mk and asserted by hack/verify-tools.sh.
