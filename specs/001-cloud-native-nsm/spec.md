...
- **SC-003**: During a 60-minute test at whatever sustained traffic rate the installation produces, active sources remain available and report less than 1% packet loss at the capture boundary; the evidence reports the measured packet rate and, when available, observed byte rate rather than assuming a reference load.
...
- **SC-005**: Across ten timed attempts using five deterministic exactly-correlatable sessions—one attempt beginning from the earliest record and one from the latest record in each session—an analyst finds the exact matching observation in under 3 minutes in all ten attempts. Timing begins when the source record opens and ends when the exact-match records are displayed; only durations, counts, and pass/fail results are retained as evidence. This sample proves Community ID pivots from both flow directions; it does not represent ten complete signature-to-protocol session pairs.
...
- A dedicated receiving interface is required for a physical-mirror traffic source. PortMirror now configures supported RouterOS 7 switches through HTTPS REST; other physical mirrors remain externally configured until a matching provider is delivered.
